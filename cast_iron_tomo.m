...
threshold = 0.5356; %lowest limit in imagej divided by max limit 0.4591
%binarise
bin = data;
bin(bin<threshold) = 0;
bin(bin>threshold) = 1;
...
%% Filter based on label volume

%first measure the size of the labelled features
stats = regionprops3(labels,'Volume');
vol = [stats.Volume];
%filter out based on the volume of the objects (here filtering out the small and large objects)
allowableAreaIndexes = (vol > 1e5) & (vol < 4e5);
keeperIndexes = find(allowableAreaIndexes);
keeperLabelsImage = ismember(labels, keeperIndexes);
%relabel
newGrains = bwlabeln(keeperLabelsImage);
% remove the objects touching the image border and relabel again:
tmp = imclearborder(newGrains);
newGrains = bwlabeln(tmp);
